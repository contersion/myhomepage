...
export async function POST(request: NextRequest) {
  try {
    ...
    // 检查是否处于冷却期
    const lockStatus = await checkLoginLocked(ip, "admin");
    if (lockStatus.locked) {
      ...
      prisma.operationLog.create({
        data: {
          userType: "admin",
          action: "LOGIN_FAILED",
          details: `管理员登录被拒绝（冷却中，剩余 ${lockStatus.remainingMinutes} 分钟）`,
          ip,
        },
      }).catch(() => {});
    ...
    // 获取管理员密码哈希
    const hash = await getPasswordHash("admin");
    ...
    if (!isValid) {
      // 记录失败次数
      const failureResult = await recordLoginFailure(ip, "admin");
      ...
      prisma.operationLog.create({
        data: {
          ...
          details: failureResult.locked 
            ? `管理员密码错误（已达最大尝试次数，锁定 ${failureResult.remainingMinutes} 分钟）`
            : `管理员密码错误（剩余 ${getRemainingAttempts(failureResult.attempts)} 次机会）`,
          ip,
        },
      }).catch(() => {});
